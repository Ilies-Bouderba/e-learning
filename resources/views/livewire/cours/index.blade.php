<div class="manage-cours-page">
    <div class="container">
        ...
        <div class="mc-filters">
            <input type="text" class="mc-search" wire:model.live.debounce.300ms="search" placeholder="Search courses...">
        ...
        <div class="mc-modal-overlay">
            <div class="mc-modal">
                ...
                <div class="mc-modal-actions">
                    ...
                    <button class="btn btn-danger" wire:click="delete"><span wire:loading.remove wire:target="delete">Yes, Delete</span><span wire:loading wire:target="delete">Deleting...</span></button>
                </div>
            </div>
        </div>
        @endif
        ...
        @if($courses->isEmpty())
        <div class="mc-empty"><span>📭</span><p>No courses found. <a href="{{ route('teacher.cours.create') }}">Create one →</a></p></div>
        @else
        <div class="mc-table-wrap">
            <table class="mc-table">
                ...
                <tbody>
                    ...
                    <tr class="mc-row">
                        <td><div class="mc-course-name"><span class="mc-course-icon">{{ $course->icon }}</span><span>{{ $course->title }}</span></div></td>
                        <td><span class="mc-dept-badge">{{ $course->department->icon }} {{ $course->department->name }}</span></td>
                        <td>@if($course->hasPassword())<span class="mc-lock">🔒 Yes</span>@else<span class="mc-open">🔓 No</span>@endif</td>
                        <td class="mc-date">{{ $course->created_at->format('M d, Y') }}</td>
                        <td>
                            <div class="mc-actions">
                                <a href="{{ route('cours.show', $course) }}" class="mc-btn-edit">View</a>
                                <a href="{{ route('teacher.cours.edit', $course) }}" class="mc-btn-edit">Edit</a>
